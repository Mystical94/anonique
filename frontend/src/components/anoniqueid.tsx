...
export function AnoniqueID() {
  ...
  useEffect(() => {
    ...
    setMetaAddr('F' + base58.encode(addr));
  }, [spendingKey]);
  ...
  const handleCopy = useCallback(
    (e: React.MouseEvent<HTMLButtonElement>) => {
      e.stopPropagation();
      copyTextToClipboard(metaAddr ? `https://anonique.io/#${metaAddr}` : '').then(
        () => {
          setCopied(true);
          setTimeout(() => {
            setCopied(false);
  ...
  return (
    <div className="large-block anonique-pane ">
      <div className="anonique-pane-header">
        {opened ? (
          <div className="anonique-pane-header">
            <div className="header-item" style={{ alignItems: 'center' }}>
              <div className="header-item header-title">
                <h1>Your Anonique ID</h1>
              </div>
              <button
          ...
          <div className="anonique-pane-header">
            <div className="header-item" style={{ alignItems: 'center' }}>
              <div className="block-wide">
                ...
                <p className="label anonique-id-contracted">{metaAddr}</p>
              </div>
              <button
      ...
      <div
        className="anonique-pane-body"
        style={{ display: opened ? 'block' : 'none' }}
      >
        <p className="block">Share the Anonique ID to receive funds.</p>
        <div className="anonique-pane-header buttons">
          <div className="block-wide">
            <p className="label" style={{ textTransform: 'uppercase' }}>
              Anonique ID:
            </p>
            <p className="label anonique-id">{metaAddr}</p>
          </div>
        ...
        <div
          className="anonique-pane-header block message"
          style={{
            width: '90%',
