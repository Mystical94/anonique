...
export function Send() {
  ...
  const [filecoinAddr, setFilecoinAddr] = useState<string>(
  ...
  const [anoniqueIDError, setAnoniqueIDError] = useState<boolean>(false);
  ...
  const debouncedAddr = useDebounce(filecoinAddr, 500);
  ...
  useEffect(() => {
    ...
    if (theirID.startsWith('https://anonique.io/#')) {
      setTheirID(theirID.replace('https://anonique.io/#', ''));
    } else {
      generateNewEphKey();
    }
  ...
  return (
    <div style={{ paddingTop: '1rem' }}>
      <p>
        ...
        controlled by the owner of Anonique ID.
      </p>
      <form
        className="lane"
        onSubmit={() => {
      ...
      >
        <div className="input-container">
          <input
            ...
            spellCheck="false"
            autoComplete="off"
            placeholder="Enter receiver Anonique ID"
            onChange={handleIDInput}
          />
          <label htmlFor="anoniqueID">ANONIQUE ID</label>
        </div>
      </form>
      ...
      {isConnected && balance && (
        <>
          <div>
            ...
            >
              ...
              <button
                className="hbutton"
                color="success"
                disabled={!write || isLoading || amountError || anoniqueIDError}
                onClick={(e) => {
                  e.preventDefault();
                  write?.();
                }}
          ...
          {anoniqueIDError && (
            <div className="lane">
              <p className="message error">Invalid Anonique ID</p>
            </div>
          )}
          {isSuccess && !isError && !isPrepareError && (
            <div className="lane">
              <p className="message">
                ...
                <a
                  href={`https://calibration.filfox.info/en/tx/${data?.hash}`}
                  className="link-text"
                  target="_blank"
                  rel="noreferrer"
                >
